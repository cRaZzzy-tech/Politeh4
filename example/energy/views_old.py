...
def step_1(request, pk):
    ...
    bbb.delete()
    with open('static/media/'+str(file.file), "r") as file :
        print(file)
        for line in file :
            # print(i,')',line)
            i+=1
            ...
            if i == 1:
                ...
                last_y = 0
                last_z = 0
                lst.append([first, float(second), float(third), float(fourth), 1])

            print(first, second, third, fourth)
            frs = func(second - last_x)
            sec = func(third - last_y)
            thr = func(fourth - last_z)

            itog = frs+sec+thr
            # print(first, 'x - ', second, ' - ', last_x, ' = ', frs)
            # print(first, 'y - ', third, ' - ', last_y, ' = ', sec)
            # print(first, 'z - ', fourth, ' - ', last_z, ' = ', thr)
            if itog < 0.5:
                # print(first, frs, sec, thr, itog)

            # if round(float(str(second)[:second.find('.')]),1) == last_x \
            #         and round(float(str(third)[:third.find('.')]),1) == last_y \
            #         and round(float(str(fourth)[:fourth.find('.')]),1) == last_z:
                lst[0][1] = lst[0][1] + float(second)
                lst[0][2] = lst[0][2] + float(third)
                lst[0][3] = lst[0][3] + float(fourth)
                lst[0][4] += 1
            elif i != 1:
                FilesTbl.objects.create(
                    parent=Files.objects.get(pk=pk),
                    x=round(lst[0][1] / lst[0][4], 3),
                    y=round(lst[0][2] / lst[0][4], 3),
                    z=round(lst[0][3] / lst[0][4], 3),
                    name=lst[0][0],
                    old_num = i
                )
                lst=[[first, float(second), float(third), float(fourth), 1]]

            last_x = second
            last_y = third
            last_z = fourth
            # last_x = float(str(second)[:second.find('.')])
            # last_y = float(str(third)[:third.find('.')])
            # last_z = float(str(fourth)[:fourth.find('.')])
            # last_x = round(float(str(second)[:second.find('.')]),1)
            # last_y = round(float(str(third)[:third.find('.')]),1)
            # last_z = round(float(str(fourth)[:fourth.find('.')]),1)
    ...
    #             f = lstfnl[-1]
    #             print(('f', f))
    #             f.x = round((f.x + line.x)/2,3)
    #             f.y = round((f.y + line.y)/2,3)
    #             f.z = round((f.y + line.z)/2,3)
    #             f.save()
    #             line.delete()
    #         else:
    #             lstfnl.append(line)
    ...
    for line in step_1:
        ...
        if i == 0:
            ...
            num3 = num1 + num2
            num4 = cmath.sqrt(num3)
            num5 = float(str(num4)[1:str(num4).find('.')+5])

            numl1 = (first_y-line.y)**2
            numl2 = (first_x-line.x)**2
            numl3 = numl1 + numl2
            numl4 = cmath.sqrt(numl3)
            numl5 = float(str(numl4)[1:str(numl4).find('.')+5])
            # num5 = sqrt(num3)
            # print(num1, num2, num3, num4, num5)
            # print(type(sqrt((prev_y-line.y)**2-(prev_x-line.x)**2)), (prev_y-line.y)**2-(prev_x-line.x)**2)
            # print(type(sqrt((prev_y-line.y)**2-(prev_x-line.x)**2)), (prev_y-line.y)**2-(prev_x-line.x)**2))
            # line.s = sqrt((prev_y-line.y)**2-(prev_x-line.x)**2)
            result = FilesTbl.objects.get(pk=line.id)
            result.s = num5
            result.l = numl5
            # result.old_num = i
    ...
    #         frst = round(float(str(second)[:second.find('.')]) - last_x, 1)
    #         scnd = round(float(str(third)[:third.find('.')]) - last_y, 1)
    #         thrd = round(float(str(fourth)[:fourth.find('.')]) - last_z, 1)
    ...
    #                 print('__1__',itog)
    #             if int(str(second)[:second.find('.')]) == last_x and int(str(third)[:third.find('.')]) == last_y \
    #                     and int(str(fourth)[:fourth.find('.')]) == last_z:
    #                 lst[0][1] = lst[0][1] + float(second)
    #                 lst[0][2] = lst[0][2] + float(third)
    #                 lst[0][3] = lst[0][3] + float(fourth)
    #                 lst[0][4] += 1
    ...
    #                 FilesTbl.objects.create(
    #                     parent=Files.objects.get(pk=pk),
    #                     x=round(lst[0][1] / lst[0][4], 4),
    #                     y=round(lst[0][2] / lst[0][4], 4),
    #                     z=round(lst[0][3] / lst[0][4], 4),
    #                     name=lst[0][0],
    #                     old_num = i
    #                 )
    #                 lst=[[first, float(second), float(third), float(fourth), 1]]
...
def gps(request):
    ...
    if 'newsletter_sub' in request.POST :
        ...
        # print(date1, date2)
        docs = GPS_doc.objects.filter(date__range=(date1, date2))
